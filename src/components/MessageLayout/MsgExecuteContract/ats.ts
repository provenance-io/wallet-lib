...
export const parseAtsData = ({ msg, fundsList }: MsgExecuteContractDisplay, isNewAts = false) => {
  const type = Object.keys(msg)[0];
  ...
  const pricePerDisplayedUnitRaw = isBid ? { amount: msgData.price, denom: quoteDenom } : { amount: msgData.price, denom: quoteDenom };
  const totalPriceRaw = isBid
